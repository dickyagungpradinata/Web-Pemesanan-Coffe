...
if(isset($_POST['simpanpelanggan'])){
    ...
    $update = "UPDATE meja set stts_meja = 'Tidak Tersedia' where no_meja = '$nomeja'";
    $up = mysqli_query($koneksi,$update);
    $query = "INSERT INTO pelanggan VALUES ('$hasilid','$nama')";
    $simpan = mysqli_query($koneksi, $query);
    
    if($simpan){
        $tanggalnow = date('Y-m-d');
        $idmeja = "SELECT id_meja FROM meja WHERE no_meja = '$nomeja'";
        $cek = mysqli_query($koneksi, $idmeja);
        $hsl = mysqli_fetch_array($cek);
        $meja = $hsl['id_meja'];
       
        $queryp = "INSERT INTO pemesanan VALUES ('$idpemesanan','$tanggalnow','$meja','$hasilid','4')";
        $simpanpesanan = mysqli_query($koneksi, $queryp);
        $temp = $idpemesanan;
        if($simpanpesanan) {
            ?>
            <script>
                var id = '<?php echo $temp ?>';
                                        ...
                                        <div class="thumbnail">
                                            <div class="caption">
                                                ...
                                                if($value['stts_meja'] == "Tersedia" ) {
                                                    ?>
                                                <h3><button onclick="tampilmodal(<?php echo $value['no_meja']; ?>);"  class="meja"><?php echo $value['no_meja']; ?></button></h3>
                                                <?php
                                                }else {
                                                    ?>
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <div class="form-group">
                                        ...
                                        <input readonly="yes" class="form-control" name="nomeja" id="nomeja"></input>
                                    </div>
                                    <div class="form-group">
