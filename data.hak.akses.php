...
if ($aksi == "save") {
    ...
    $query = "INSERT INTO hak_akses (nm_akses) VALUES('$nm_akses')";
    $hasil = $dao->execute($query);
    if ($query) {
        header("location:data.hak.akses.php");
    } else {
        echo 'Data Gagal Disimpan';
    }
} else if ($aksi == "edit") {
    $query = "UPDATE `hak_akses` SET `nm_akses`='$nm_akses' WHERE `id_akses`='$id_akses';";
    $hasil = $dao->execute($query);
    if ($query) {
        header("location:data.hak.akses.php");
    } else {
    ...
    $query = "DELETE FROM `hak_akses` WHERE `id_akses`='$id_akses';";
    $hasil = $dao->execute($query);
    if ($query) {
        header("location:data.hak.akses.php");
    } else {
...
<html lang="en">
    <head>
        <title>Hak Akses- APLIKASI PEMESANAN E-MENU</title>
        <meta charset="utf-8">
        ...
        <style>
         ...
         <script>
            function showModalEdit(id_akses,nm_akses) {
                document.getElementById("id_akses").readOnly = true; 
                document.getElementById("nm_akses").readOnly = false;
                
                $('#tombol').text('Edit');
                $('#aksi').val('edit');
                $('#id_akses').val(id_akses);
                $('#nm_akses').val(nm_akses);
                $('#myModal').modal('show');
            }
            function showModalSave() {
                document.getElementById("id_akses").readOnly = false; 
                document.getElementById("nm_akses").readOnly = false;
            ...
            function showModalDel(id_akses,nm_akses) {
                document.getElementById("id_akses").readOnly = true; 
                document.getElementById("nm_akses").readOnly = true;
                                ...
                                <table class="table table-hover">
                                    ...
                                    foreach ($result as $value) {
                                     ?>
                                        ...
                                        <td><?php echo $value['nm_akses']; ?></td>
                                        <td>
                                          <button class="btn btn-warning" type="button" onclick="showModalEdit
                                                                ('<?php echo $value['id_akses']; ?>',
                                                                 '<?php echo $value['nm_akses']; ?>');"><span class="glyphicon glyphicon-pencil"></span> Edit</button>
                                            <button class="btn btn-danger" type="button" onclick="showModalDel
                                                              ('<?php echo $value['id_akses']; ?>',
                                                               '<?php echo $value['nm_akses']; ?>');"><span class="glyphicon glyphicon-remove"></span> Delete</button>
                                    </tr>
                                    <?php
                                    }
                                    ?>
                    ...
                    <div class="modal-dialog modal-md">
                        <div class="modal-content">
                            <div class="modal-header">
                                ...
                                <h4 class="modal-title">INPUT DATA HAK AKSES</h4>
                            </div>
                          <div class="modal-body">
                             <div class="modal-body">
                                <div class='content'>
                                    <section>
                                        <form method ="POST" action="data.hak.akses.php"> 
                                            <input class="form-control" name="id_akses" id="id_akses" type="hidden">
                                            <div class="form-group">
                                                <input type="hidden" id="aksi" name="aksi">
                                                <label>Nama Hak Akses</label>
                                                <input name ="nm_akses" type="text" id="nm_akses" class="form-control" placeholder="Masukkan Data" required="">
                                            </div>
